...
function sfxBreath(){
  ...
  src.buffer = buf;
  g.gain.setValueAtTime(0, t); g.gain.linearRampToValueAtTime(.12, t+.2); g.gain.linearRampToValueAtTime(0, t+.6);
...
function startMusic(){
  ...
  function playMel(){
    MELODY.forEach((idx,i)=>{
      ...
      const o = ac.createOscillator(), g = ac.createGain();
      o.type='sine'; o.frequency.setValueAtTime(freq,t+i*step); o.frequency.setValueAtTime(freq*1.002,t+i*step+.01);
      g.gain.setValueAtTime(0,t+i*step); g.gain.linearRampToValueAtTime(.095,t+i*step+.04);
      g.gain.setValueAtTime(.095,t+i*step+step*.55); g.gain.exponentialRampToValueAtTime(.001,t+i*step+step*.9);
      o.connect(g); g.connect(masterGain);
      o.start(t+i*step); o.stop(t+i*step+step); musicNodes.push(o);

      const o2 = ac.createOscillator(), g2 = ac.createGain();
      o2.type='sine'; o2.frequency.value=freq*.5;
      g2.gain.setValueAtTime(0,t+i*step); g2.gain.linearRampToValueAtTime(.038,t+i*step+.04);
      g2.gain.exponentialRampToValueAtTime(.001,t+i*step+step*.7);
      o2.connect(g2); g2.connect(masterGain);
      o2.start(t+i*step); o2.stop(t+i*step+step); musicNodes.push(o2);
    });
    ...
    BASS.forEach((idx,i)=>{
      ...
      const o = ac.createOscillator(), g = ac.createGain();
      o.type='sine'; o.frequency.value=freq;
      g.gain.setValueAtTime(0,t+i*step*1.35); g.gain.linearRampToValueAtTime(.055,t+i*step*1.35+.05);
      g.gain.exponentialRampToValueAtTime(.001,t+i*step*1.35+step*1.2);
      o.connect(g); g.connect(masterGain);
      o.start(t+i*step*1.35); o.stop(t+i*step*1.35+step*1.3); musicNodes.push(o);
    });
...
function stopMusic(){
  ...
  setTimeout(()=>{musicNodes.forEach(n=>{try{n.stop()}catch(_){}}); musicNodes=[];},700);
  musicPlaying=false;
}
...
function toggleMusic(){
  if(musicPlaying){
    ...
    document.getElementById('music-label').textContent='Musique ♪ Pause';
    document.getElementById('music-bars').style.opacity='.3';
    sfxBell(.08);
  } else {
    startMusic();
    document.getElementById('music-btn').textContent='🎵';
...
function burst(lx,ly){
  ...
  for(let i=0;i<38;i++){
    ...
    const angle=Math.random()*Math.PI*2, dist=Math.random()*280+80, sz=Math.random()*15+7;
...
function initRoses(){
  ...
  ROSES.forEach((r,i)=>{
    const card=document.createElement('div'); card.className='rose-card'; card.style.animationDelay=r.delay+'s';
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Boutons
  document.getElementById('stress-btn').addEventListener('click', handleStress);
  const merciBtn=document.getElementById('merci-btn');
  if(merciBtn) merciBtn.addEventListener('click', closeCalmCard);

  const musicBtn=document.getElementById('music-btn');
  if(musicBtn) musicBtn.addEventListener('click', toggleMusic);

  const modal=document.getElementById('modal');
  if(modal) modal.addEventListener('click', (e)=>{if(e.target===modal) closeModalBtn();});

  // Auto start musique au premier clic/tap
